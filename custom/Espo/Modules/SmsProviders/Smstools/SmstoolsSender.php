...
namespace Espo\Modules\SmsProviders\Smstools;
...
class SmstoolsSender implements Sender
{
    ...
    private function sendToNumber(Sms $sms, string $toNumber): void
    {
        ...
        $clientId = $integration->get('smstoolsClientId');
        $clientSecret = $integration->get('smstoolsClientSecret');
        $baseUrl = rtrim(
            $integration->get('smstoolsBaseUrl') ??
            $this->config->get('smstoolsBaseUrl') ??
            self::BASE_URL
        );

        $sender = $integration->get('smstoolsSender');
        $reference = $integration->get('smstoolsReference');
        $test = $integration->get('smstoolsTest');

        $timeout = $this->config->get('smstoolsSmsSendTimeout') ?? self::TIMEOUT;

        if (!$clientId) {
            throw new Error("No Smstools Client Id.");
        }

        if (!$clientSecret) {
            throw new Error("No Smstools Client Secret.");
        }
        ...
        if ($error) {
            if (in_array($error, [\CURLE_OPERATION_TIMEDOUT, \CURLE_OPERATION_TIMEOUTED])) {
                throw new Error("Smstools SMS sending timeout.");
            }
        }
    }
    ...
    private function processError(int $code, string $body): void
    {
        ...
        if ($message) {
            $this->log->error("Smstools SMS sending error. Message: " . $message);
        }

        throw new Error("Smstools SMS sending error. Code: {$code}.");
    }

    private function getIntegrationEntity(): Integration
    {
        $entity = $this->entityManager
            ->getEntity(Integration::ENTITY_TYPE, 'Smstools');

        if (!$entity || !$entity->get('enabled')) {
            throw new Error("Smstools integration is not enabled");
        }
